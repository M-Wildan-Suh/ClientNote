            ...
            <div x-data="{ modal: false, deletemodal: false }">
                <div
                    ...
                    <div
                        ...
                        {{-- <textarea id="keterangan" data-autoresize oninput="autoResizeTextarea(this)" name="description" id=""
                                        ...
                                        class=" text-xs sm:text-base w-full p-0 overflow-hidden resize-none border-none ring-0 focus:border-none focus:ring-0">{{ $data->description }}</textarea> --}}
                        @if ($data->image)
                            <a data-fancybox="gallery" aria-label="Gallery"
                                href="{{ asset('/storage/images/note/' . $data->image) }}" class="flex w-full">
                                <button
                        ...
                        {{-- <div class="w-full rounded-md bg-black overflow-hidden">
                                    <img src="{{ asset('/storage/images/note/' . $data->image) }}" alt="">
                                </div> --}}
                        <div class=" flex flex-col gap-2">
                            ...
                            <div class=" w-full flex justify-end gap-2">
                                <div x-data="{ copied: false, original: '{{ route('note', ['id' => $data->id]) }}' }" class=" relative">
                                    <button type="button"
                                ...
                                <a href="https://wa.me/{{ $data->no_tlp }}" target="__blank">
                                    <button @click="modal = !modal" type="button" {{ $data->no_tlp ? '' : 'disabled' }}
                                        class=" {{ $data->no_tlp ? 'bg-white hover:bg-green-200 text-green-500 border-green-500' : 'bg-white text-neutral-500 border-neutral-500' }} w-7 border aspect-square p-1 overflow-hidden rounded-full font-semibold duration-300">
                ...
                <div x-show="modal" class=" fixed inset-0 flex justify-center items-center bg-black/20 p-4 z-50">
                    <div
                        ...
                        <form action="{{ route('webnote.update', ['webnote' => $data->id]) }}" method="POST"
                            ...
                            <div class="text-sm sm:text-base p-4 space-y-4">
                                ...
                                <div class=" flex flex-col gap-2">
                                    ...
                                    <textarea id="description" data-autoresize oninput="autoResizeTextarea(this)" placeholder="Masukkan Keterangan"
                                        ...
                                        name="description" id="" rows="1">{{ $data->description }}</textarea>
                                </div>
                                <div x-data="{ fileName: '{{ $data->image }}' }" class="flex flex-col gap-2">
                                    <label for="image-{{ $data->id }}">Gambar</label>
                                    ...
                                    <input type="file" name="image" id="image-{{ $data->id }}"
                                        class="hidden"
                                ...
                                <div class=" space-y-2">
                                    ...
                                    <input type="text" name="no_tlp" id="no_tlp"
                                        placeholder="Masukkan No Telephone" value="{{ $data->no_tlp }}"
                                        class=" text-sm sm:text-base w-full py-1 focus:border-[#0A2342] focus:ring-[#0A2342] duration-300 rounded-md overflow-hidden">
                                </div>
                                <div class=" space-y-2">
                                    <label>Status</label>
                                    <div class=" w-full grid grid-cols-4 gap-1">
                                        <div class=" flex">
                                            <input type="radio" name="status" id="stat-new"
                                                {{ $data->status === 'Prospek Baru' ? 'checked' : '' }}
                                        ...
                                        <div class=" flex">
                                            <input type="radio" name="status" id="stat-progress"
                                                {{ $data->status === 'Progress' ? 'checked' : '' }} value="Progress"
                                                class="peer hidden">
                                        ...
                                        <div class=" flex">
                                            <input type="radio" name="status" id="stat-done"
                                                {{ $data->status === 'Selesai' ? 'checked' : '' }} value="Selesai"
                                                class="peer hidden">
                                            <label for="stat-done"
                                        ...
                                        <div class=" flex">
                                            <input type="radio" name="status" id="stat-cancel"
                                                {{ $data->status === 'Cancel' ? 'checked' : '' }} value="Cancel"
                                                class="peer hidden">
                ...
                <div x-show="deletemodal" class=" fixed inset-0 flex justify-center items-center bg-black/20 p-4">
                    <div
                        class=" w-full max-w-xl bg-white border border-[#0A2342] rounded-md divide-y divide-[#0A2342] overflow-hidden">
                        <div
                            class=" flex items-center justify-between text-white bg-[#0A2342] text-base sm:text-lg font-semibold pt-4 px-4 pb-2">
                            <p>Hapus Note</p>
                            <button @click="deletemodal = !deletemodal" class=" w-4 sm:w-5 aspect-square text-white">
                                <svg viewBox="0 0 36 36" xml:space="preserve" xmlns="http://www.w3.org/2000/svg"
                                    enable-background="new 0 0 36 36">
                                    <g id="icons">
                                        <path
                        ...
                        <form action="{{ route('webnote.destroy', ['webnote' => $data->id]) }}" method="POST">
                            @csrf
                            @method('delete')
